...
instance Parseable Exp where
  parse (t:ts) =
    if t == "max" && length ts > 0 && ts!!0 == "(" then
      ...
      Just (Variable t, ts)
    else if t == "DATA" then Just(DATA, ts)
    else
      Nothing
  parse _ = Nothing  
...
instance Parseable Stmt where
  parse (t:ts) =
    if t == "return" && ts!!0 `subset` "abcdefghijklmnopqrstuvwxyz" && ts!!1 == ";" then
      Just (Return (ts!!0), [])

    -- Add missing cases for Problem 1, part (a).
...
example = fst $ (\(Just x)->x) $ parse (tokenize "assign t = sum(DATA); return t;") :: Stmt
