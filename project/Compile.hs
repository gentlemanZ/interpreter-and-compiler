...
simulate (MakeSets       :insts) kvs = [(a, Set [b]) | (a, Number b)<- kvs] -- Complete for Problem 5, part (a).
simulate []                      kvs = kvs

compile :: Stmt -> KeyValueStore -> Maybe KeyValueStore
compile stmt kvs = if (typeCheck [] stmt) /= Nothing then Just ( simulate (compileStmt (rmvDeadCode(stmt))) kvs) else Just [] -- Complete for Problem 5, part (b).
