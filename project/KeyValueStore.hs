...
twoWithSameKeyHelper ((k1,v1):(k2,v2):kvs) keep  =
  ...
  else twoWithSameKeyHelper([(k2,v2)]++kvs) (keep++[(k1,v1)])  -- Complete recursive case for Problem 3, part (a).
twoWithSameKeyHelper _                     _     = Nothing

twoWithSameKey :: (Ord a, Ord b, Eq a) => [(a,b)] -> Maybe (a, b, b, [(a,b)])
twoWithSameKey kvs = (twoWithSameKeyHelper (sort(kvs)) []) -- Complete for Problem 3, part (b).

suffix :: [([a],b)] -> [([a],b)]
suffix kvs = [(tail (xs),b)| (xs,b)<-kvs] -- Complete for Problem 3, part (c).
